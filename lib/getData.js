...
export const getPopularTvData = async (page) => {
  // Call an external API endpoint to get posts
  const res = await fetch(
    `https://api.themoviedb.org/3/tv/popular?api_key=403829fffc80d8184aa974d631a475c5&language=en-US&page=${
      page | 1
    }`
  );
  const popularData = await res.json();
...
export const getSimilarData = async (id) => {
  const similar = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${process.env.apiKey}&language=en-US&page=1`
  );
...
export const getTvPicsData = async (id) => {
  const pics = await fetch(
    `https://api.themoviedb.org/3/tv/${id}/images?api_key=${process.env.apiKey}`
  );
  const pic = await pics.json();
  const picSelected = pic.posters;
  return picSelected;
...
export const getTvVideosData = async (id) => {
  const videos = await fetch(
    `https://api.themoviedb.org/3/tv/${id}/videos?api_key=${process.env.apiKey}`
  );
  const vid = await videos.json();
  const videoSelected = vid.results;
  return videoSelected;
...
export const getTvSimilarData = async (id) => {
  const similar = await fetch(
    `https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=${process.env.apiKey}&language=en-US&page=1`
  );
  const similarDataRes = await similar.json();
  const similarData = similarDataRes.results;
  return similarData;
...
export const getTvCreditData = async (id) => {
  try {
    const credits = await fetch(
      `https://api.themoviedb.org/3/tv/${id}/credits?api_key=${process.env.apiKey}`
    );
    const credit = await credits.json();
    const casts = credit.cast;
    const crews = credit.crew;
