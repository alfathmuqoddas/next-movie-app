export const getPopularData = async (type, page) => {
  ...
  try {
    const res = await fetch(
      `https://api.themoviedb.org/3/${type}/popular?api_key=403829fffc80d8184aa974d631a475c5&language=en-US&page=${
        page | 1
      }`
    );
    if (!res.ok) {
      throw new Error("Failed to fetch ow playing data");
    }
...
export const getPicsData = async (type, id) => {
  try {
    const pics = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}/images?api_key=${process.env.apiKey}`
    );
    if (!pics.ok) {
...
export const getVideosData = async (type, id) => {
  try {
    const videos = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}/videos?api_key=${process.env.apiKey}`
    );
    if (!videos.ok) {
...
export const getSimilarData = async (type, id) => {
  try {
    const similar = await fetch(
      `https://api.themoviedb.org/3/${type}/${id}/recommendations?api_key=${process.env.apiKey}&language=en-US&page=1`
    );
    if (!similar.ok) {
