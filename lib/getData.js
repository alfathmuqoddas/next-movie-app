...
export const getDiscover = async (genreId, page, mediaType) => {
  try {
    const discoverData = (await ((await fetch(`https://api.themoviedb.org/3/discover/${mediaType}?api_key=${process.env.API_KEY}&include_adult=false&include_video=false&language=en-US&page=${page}&sort_by=popularity.desc&with_genres=${genreId}`)).json()));
    return discoverData
  } catch (err) {
